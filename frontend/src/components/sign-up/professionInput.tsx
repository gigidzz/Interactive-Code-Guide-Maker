...
export const ProfessionInput: React.FC<ProfessionInputProps> = ({
  ...
  <div>
    <label htmlFor="profession" className="block text-sm font-medium text-purple-700">
      Professions
    </label>
    <div className="mt-1">
      <div className="flex gap-2">
        <input
          ...
          className={`appearance-none block flex-1 px-3 py-2 border rounded-md placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500 ${
            error ? 'border-purple-300' : 'border-gray-300'
          }`}
        ...
        <button
          type="button"
          onClick={onAdd}
          className="px-3 py-2 bg-purple-600 text-white text-sm rounded-md hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
          Add
        </button>
      </div>
      ...
      {professions.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-2">
          {professions.map((prof, index) => (
            <span
              key={index}
              className="inline-flex items-center gap-1 px-2 py-1 bg-purple-800 text-white text-sm rounded-md"
            >
              {prof}
              <button
                type="button"
